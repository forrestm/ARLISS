...
xbee.write('target point: {}'.format(finish_point))
xbee.write('\nlaunching at: {}'.format(launch_point))
...
while True:
    ...
    if new_data:
        ...
        if my_gps.latitude[0] != 0:
            ...
            if dist_from_launch < 10:
                # pyb.stop()
                pyb.delay(100)
            elif dist_from_launch > 10:
                altitude_1 = my_gps.altitude
                pyb.delay(2000) # change to 10000 for real launch
                altitude_2 = my_gps.altitude
                altitude_change = abs(altitude_1 - altitude_2)
                if altitude_change > 10:
                    # pyb.stop()
                    pyb.delay(100)
                elif altitude_change < 10: #and pyb.elapsed_millis(start) < ___: #acceptable time yet to be determined
                    break
            # elif dist_from_launch < 10 #and pyb.elapsed_millis(start) < ___: #acceptable time yet to be determined; will be different than above time
                # break
            new_data = False
            past_point2 = None
...
xbee.write('\n\nI have landed at {}'.format(initial_point))
